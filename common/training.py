                    ...
                    try:
                        if progress_bar.n <= -1 and mode == "train":
                            ...
                            for i in range(len(batch["indexes"])):
                                noteindex = noteindex + str(batch["indexes"][i]) + str(batch["start"][i]) + str(len(batch["label"]))
                            #todo 删除测试代码
                            ...
                            if kwargs["note_use"] and mode == "train":
                                ...
                                if count_mode == "sample":
                                    batch_size = batch_metrics["seq_total"] if "seq_total" in batch_metrics else batch_len
