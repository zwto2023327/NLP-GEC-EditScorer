...
class ModelTrainer:
                                    ...
                                    for i in range(fir):
                                        ...
                                        if sec != 0:
                                            for index in range(sec):
                                                if self.notelist[noteindex][index_list[num]] == 0 and batch_metrics["error_list"][i][index] == 1 and (num + 1) not in batch["offset"] and self.testvalidate == 1:
                                                    epochfilet.write("***")
                                                    epochfilet.write(batch['target'][num])
                                                if self.correctflag > 0 :
                                                    if self.testvalidate == 1:
                                                        ra = random.randint(0, 100) / 100
                                                        if noteindex in self.correctlist and index_list[num] in self.correctlist[noteindex] and ra <= 0.1:
                                                                ...
                                                                if batch['flag'][num] == 0:
                                                                    if batch['target'][num] == '':
                                                                        ...
                                                                        if batch_metrics["error_list"][i][index] == 1:
                                                                            ...
                                                                            if self.notelist[noteindex][index_list[num]] == 0:
    ...
    def train(self, model, train_data, dev_data=None, total=None, dev_total=None, count_mode="sample", **kwargs):
        ...
        file_path = "/home/boot/wzx/error_epoch.txt"
        if os.path.exists(file_path):
            os.remove(file_path)
        file = open(file_path, "a")
        self.txt_path = "/home/boot/wzx/epoch_txt/"
        self.valtxt_path = "/home/boot/wzx/epoch_val_txt/"
        self.traintxt_path = "/home/boot/wzx/epoch_train_txt/"
